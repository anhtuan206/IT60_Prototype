...
const AdminSettings = () => {
  ...
  return (
    <AdminLayout title="Cài đặt" breadcrumbs={breadcrumbs}>
      <div className="bg-white border border-gray-200 rounded-md p-6">
              ...
              {roles.map((role) => (
                <li key={role.id}>
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-md ${selectedRole === role.id ? 'bg-accent-beige font-semibold' : 'hover:bg-gray-50'}`}
                  >
                    {role.name}
                  </button>
                </li>
              ...
              <table className="w-full min-w-[600px]">
                <thead>
                  <tr className="border-b">
                    ...
                    {permissions.map((p) => (
                      <th key={p} className="text-center font-medium py-2">
                        {p}
                      </th>
                    ))}
                  </tr>
        ...
        <div className="mt-8 pt-4 border-t border-gray-200 flex justify-end">
          <button
            onClick={handleSave}
            className="px-4 py-2 bg-primary-blue text-white rounded-md hover:opacity-90 flex items-center"
          >
