...
const AdminProductsList = () => {
  ...
  return <AdminLayout title="Sản phẩm" breadcrumbs={breadcrumbs}>
      ...
      <div className="mb-4">
        <Link to="/admin/products/new" className="inline-flex items-center bg-amber-700 text-white px-4 py-2 rounded-md hover:opacity-90">
          <Plus size={16} className="mr-2" />
          Thêm Sản Phẩm
        </Link>
      </div>

      <DataTable columns={columns} data={products} selectable selectedRows={selectedRows} onSelectRow={handleRowSelect} onSelectAll={handleSelectAll} onRowClick={row => {
      ...
      <ConfirmModal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} onConfirm={handleDelete} title="Xác nhận xóa" message={`Bạn có chắc chắn muốn xóa sản phẩm "${selectedProduct?.name || ''}"?`} />
    </AdminLayout>;
};
