...
const AdminUsersList = () => {
  ...
  const users = [{
    ...
    id: '#U-003',
    name: 'Lê Hoàng (Admin)',
    email: 'lehoang.admin@pethotel.com',
    role: 'admin',
    status: 'active',
    created: '15/09/2025'
  }, {
    id: '#U-004',
    name: 'Phạm Minh (Staff)',
    email: 'phamminh.staff@pethotel.com',
    role: 'staff',
    status: 'disabled',
    created: '12/09/2025'
  }, {
  ...
  return <AdminLayout title="Người dùng" breadcrumbs={breadcrumbs}>
      ...
      <div className="mb-4">
        <Link to="/admin/users/new" className="inline-flex items-center bg-amber-700 text-white px-4 py-2 rounded-md hover:opacity-90">
          <UserPlus size={16} className="mr-2" />
          Thêm Người Dùng
        </Link>
      </div>

      <DataTable columns={columns} data={users} selectable selectedRows={selectedRows} onSelectRow={handleRowSelect} onSelectAll={handleSelectAll} onRowClick={row => {
      ...
      <ConfirmModal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} onConfirm={handleDelete} title="Xác nhận xóa" message={`Bạn có chắc chắn muốn xóa người dùng "${selectedUser?.name || ''}"?`} />
    </AdminLayout>;
};
